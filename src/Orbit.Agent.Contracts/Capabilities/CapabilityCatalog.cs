...
public static class CapabilityCatalog
{
    ...
    [
        ...
        new() { Id = "emails.outlookPush", Route = "Orbit.App/Emails", Status = "available", Notes = "Out-of-process Classic Outlook selection → SaveAs .msg → ingest (ADR 0024). Memo dialog in App." },
        new() { Id = "emails.outlookLauncher", Route = "orbit://push-outlook", Status = "available", Notes = "Thin Outlook ribbon ShellExecutes orbit://push-outlook; Orbit App does the pull (Orbit.OutlookLauncher)" },
        new() { Id = "emails.outlookWebAddIn", Route = "/v1/emails/from-outlook", Status = "deprecated", Notes = "Parked Office.js+Vite experiment; prefer emails.outlookLauncher" },
        new() { Id = "emails.outlookAddIn", Route = "Orbit.OutlookAddIn", Status = "deprecated", Notes = "In-proc COM ingest — clr.dll AV; do not register" },
        ...
        new() { Id = "calendar.sources", Route = "/v1/calendar/sources", Status = "available", Notes = "List distinguishable mailbox/calendar sources; PATCH /{id} toggles enabled include/exclude" },
        ...
        new() { Id = "agent.tools.orbit_update_task", Route = "/v1/agent/tools/orbit_update_task", Status = "available", Notes = "Typed mutation; move via projectId/workstreamId; audited" },
        new() { Id = "agent.tools.orbit_create_project", Route = "/v1/agent/tools/orbit_create_project", Status = "available", Notes = "Create project; 409 + candidates on near-dupe unless force=true" },
        new() { Id = "agent.tools.orbit_update_project", Route = "/v1/agent/tools/orbit_update_project", Status = "available", Notes = "Update name/summary/code/aliases/accentColor; audited" },
        new() { Id = "agent.tools.orbit_merge_project", Route = "/v1/agent/tools/orbit_merge_project", Status = "available", Notes = "Operator-initiated merge; previewOnly or force; archives source" },
        new() { Id = "agent.tools.orbit_add_project_alias", Route = "/v1/agent/tools/orbit_add_project_alias", Status = "available", Notes = "Operator nickname for matching; globally unique normalized alias" },
        new() { Id = "agent.tools.orbit_remove_project_alias", Route = "/v1/agent/tools/orbit_remove_project_alias", Status = "available", Notes = "Remove alias by text or id" },
        new() { Id = "agent.tools.orbit_list_project_aliases", Route = "/v1/agent/tools/orbit_list_project_aliases", Status = "available", Notes = "List aliases for a project" },
        ...
        new() { Id = "agent.tools.orbit_link_tasks", Route = "/v1/agent/tools/orbit_link_tasks", Status = "available", Notes = "Task dependency edge (blocks|informs|relates) with expects/follow-up; deduped + cycle-guarded; audited" },
        ...
        new() { Id = "agent.tools.orbit_satisfy_task_dependency", Route = "/v1/agent/tools/orbit_satisfy_task_dependency", Status = "available", Notes = "Clear waiting edge with evidence without deleting expects/reason" },
        new() { Id = "agent.tools.orbit_get_task_dependencies", Route = "/v1/agent/tools/orbit_get_task_dependencies", Status = "available", Notes = "waitingOn/feeds split with satisfied flags + follow-up/cadence" },
        ...
        new() { Id = "agent.tools.orbit_set_waiting_on", Route = "/v1/agent/tools/orbit_set_waiting_on", Status = "available", Notes = "Task-level waiting-on label/person/org + follow-up/cadence; preserves blocker text" },
        new() { Id = "agent.tools.orbit_clear_waiting_on", Route = "/v1/agent/tools/orbit_clear_waiting_on", Status = "available", Notes = "Satisfy waiting-on with evidence; keeps label history" },
