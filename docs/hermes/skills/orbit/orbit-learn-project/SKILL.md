...
Given a project linked to a home folder under the operator's `Projects/` tree, learn what the project actually is from indexed files and update the living brief.
...
4. `orbit_remember` durable project facts (accounts, vendors, phase) with evidence refs when possible.
